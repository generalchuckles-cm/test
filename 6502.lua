...
-- Part 2.1: Replaced bit32 library with standard math to avoid sandbox errors
...
-- ===================================================================
-- | Simple 8008 Assembler (Sandbox-Safe)                            |
-- ===================================================================
        ...
        if line ~= '' then
            ...
            if opcodes[mnemonic] then
                ...
                elseif op_info[2] == 2 then
                    ...
                    local low_byte = addr % 256
                    local high_byte = math.floor(addr / 256) % 256
...
-- ===================================================================
-- | Intel 8008 Emulator Core (Sandbox-Safe)                         |
-- ===================================================================
...
local function read_byte(addr) return memory[addr % 16384] or 0 end
local function write_byte(addr, val) memory[addr % 16384] = val % 256 end
local function get_hl() return (regs[reg_map.H] % 64) * 256 + regs[reg_map.L] end
local function update_flags(val)
    val = val % 256
    flags.Z = (val == 0) and 1 or 0
    flags.S = (val >= 128) and 1 or 0
    local p_count = 0
    for i=0,7 do if math.floor(val / (2^i)) % 2 == 1 then p_count=p_count+1 end end
    flags.P = (p_count % 2 == 0) and 1 or 0
end

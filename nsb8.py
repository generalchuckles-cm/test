...
# Part 3: Added support for file attachments and raw URLs

import sys
import urllib.request # To download code from URLs

# --- NEW INPUT HANDLER ---
# Determines the source of the code (attachment, URL, or direct input)
try:
    user_input = args
    if user_input.startswith('http'):
        # It's a URL from a file attachment or direct link
        try:
            with urllib.request.urlopen(user_input) as response:
               # Read the content of the file and decode it from bytes to a string
               assembly_code = response.read().decode('utf-8')
        except Exception as e:
            assembly_code = f"; Error fetching URL: {e}"
    else:
        # It's direct code from a code block
        assembly_code = user_input
except (NameError, IndexError):
    # Fallback if no argument is provided at all
    assembly_code = "LAI 1 ; No code provided. Attach a file, use a code block, or provide a URL."

# ===================================================================
# | Robust 8008 Assembler (No changes needed)                       |
# ===================================================================
def assemble(code):
    ...
    while i < len(tokens):
        ...
        if mnemonic in opcodes:
            ...
            if num_operands == 1: bytecode.append(int(operands))
            elif num_operands == 2:
                addr = int(operands); bytecode.append(addr & 0xFF); bytecode.append((addr >> 8) & 0xFF)
        else: return None, f"Error: Unknown mnemonic '{mnemonic}'"
    status += f"Program size: {len(bytecode)} bytes."
    return bytecode, status

# ===================================================================
# | Intel 8008 CPU Class (No changes needed)                        |
# ===================================================================
class Intel8008:
    def __init__(self):
        ...
        self.pc = 0; self.sp = 0; self.stack = * 7; self.halted = False
